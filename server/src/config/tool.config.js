...
export const availableTools = [
  ...
  },
  {
    ...
    name: "URL Context",
    description:
      "Provide specific URLs that you want the model to analyze directly from the prompt. Supports upto 20 URLs per request.",
...
export function enableTools(toolIds) {
  console.log(chalk.red("[DEBUG] enableTools call with:"), toolIds);
  ...
  const enabelCount = availableTools.filter((t) => t.enabled).length;
  console.log(
    chalk.gray(
      `[DEBUG] Total tools enabled: ${enabelCount}/${availableTools.length}`
    )
  );
}
